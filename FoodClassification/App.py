...
Original file is located at
    https://colab.research.google.com/drive/16C6maOGj0mbo_bBv3oZMfFdPIgDXyeEh
"""
...
st.title("Welcome to Multiclass Food Classification")
st.write(" ------ ")
...
for i in range(1, num_classes+1):
  image_option = 'photo ' + str(i)
  IMAGES_OPTIONS.append(image_option)
image_option = st.sidebar.selectbox("Please select from the following", IMAGES_OPTIONS)

dictionary = dict()
for i in range(1, num_classes+1):
  key = 'photo ' + str(i)
  data = test_dataset[(i-1)*1000]
  img, label = data
  predicted_label = predict_image(img, model)
  dictionary[key] = [data, predicted_label]
...
if pressed:
    ...
    img, label = dictionary[image_option][0]
    plt.imshow(img.permute(1, 2, 0))
    st.pyplot()
    st.write('Label:', test_dataset.classes[label], ', Predicted:', dictionary[image_option][1])

test_loader = DeviceDataLoader(DataLoader(test_dataset_mini, batch_size*2), device)
result = evaluate(model, test_loader)
st.write("Accuracy on whole test dataset:", result)